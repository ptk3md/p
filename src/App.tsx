...
import {
  Loader2, Save, Home, PieChart, Settings, X, AlertTriangle, FileText, RotateCcw,
...
export interface Question {
  ...
  alternativas?: any; correta: string; comentario: string; origem: string;
...
// ============================================================================
// 2. ENGINE LOCAL (Substituindo Supabase e Serviços)
// ============================================================================
const STORAGE_KEYS = {
...
const localDb = {
  ...
  saveAnswer: (item: UserHistoryItem) => {
    const hist = localDb.getHistory(); hist.push(item);
  ...
  saveProgress: (data: any) => localStorage.setItem(STORAGE_KEYS.PROGRESS, JSON.stringify(data)),
  checkDailyLimit: () => {
...
const QuizTimer = React.memo(({ isRunning, initialTime = 0, onTimeUpdate }: any) => {
  const [time, setTime] = useState(initialTime);
  useEffect(() => {
    if (!isRunning) return;
    const intervalId = setInterval(() => setTime((t: number) => { const nt = t + 1; onTimeUpdate(nt); return nt; }), 1000);
  ...
  return <div className="bg-slate-100 dark:bg-slate-800 px-2 py-0.5 rounded-md font-mono text-xs text-slate-600 dark:text-slate-300 border border-slate-200 dark:border-slate-700">{formatTime(time)}</div>;
});

// ============================================================================
// 4. EXAMS SCREEN (100% INTACTO)
// ============================================================================
const ExamsScreen: React.FC<{ questions: Partial<Question>[], history: UserHistoryItem[], onStartExam: any }> = ({ questions, history, onStartExam }) => {
  ...
  const availableExams = useMemo(() => {
    ...
    Object.entries(groups).forEach(([key, data]) => {
      if (data.count > 0) { 
        ...
        exams.push({ id: key, origem, ano, count: data.count, userCorrect: correctCount, userTotalDone: doneCount, score: doneCount > 0 ? Math.round((correctCount / doneCount) * 100) : 0, isComplete: doneCount === data.count });
      }
    });
    return exams.sort((a, b) => a.origem.localeCompare(b.origem) || Number(b.ano) - Number(a.ano));
  }, [questions, history]);

  const displayedExams = useMemo(() => (!searchTerm.trim()) ? availableExams : availableExams.filter(e => e.origem.toLowerCase().includes(searchTerm.toLowerCase()) || e.ano.includes(searchTerm.toLowerCase())), [availableExams, searchTerm]);
    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-8 pb-32 animate-in fade-in duration-500 font-sans">
      <div className="text-center py-6 max-w-2xl mx-auto space-y-6">
        <div className="animate-in slide-in-from-top-4 duration-500">
            ...
            <p className="text-slate-500 dark:text-slate-400 text-sm md:text-base leading-relaxed">Domine as bancas. Simule as condições reais de prova com o conteúdo completo organizado por ano.</p>
        </div>
        <div className="relative group z-10 animate-in zoom-in-95 duration-300">
            ...
            {searchTerm && <button onClick={() => setSearchTerm('')} className="absolute inset-y-0 right-0 pr-4 flex items-center text-slate-400 hover:text-[#BD4234] transition-colors"><X size={18} /></button>}
        </div>
      </div>

      {displayedExams.length === 0 ? (
        <div className="flex flex-col items-center justify-center py-16 px-4 text-center border border-dashed border-slate-200 rounded-2xl max-w-2xl mx-auto"><h3 className="font-semibold text-slate-900 dark:text-white mb-1">Nenhuma prova encontrada</h3></div>
      ) : (
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4 md:gap-5">
          {displayedExams.map((exam, index) => (
            <button key={exam.id} onClick={() => setSelectedExam(exam)} className="group relative bg-white dark:bg-slate-900 rounded-xl p-5 border border-slate-200 dark:border-slate-800 shadow-sm hover:border-[#BD4234]/50 transition-all flex flex-col justify-between h-[240px] text-left w-full">
              {exam.isComplete && <div className="absolute top-3 right-3 z-20 text-emerald-500 bg-emerald-50 p-1.5 rounded-md"><CheckCircle2 size={16} strokeWidth={2.5} /></div>}
              <div className="relative z-10 w-full">
                <div className="flex justify-between items-start mb-4">
                   <div className="inline-flex flex-col items-center justify-center w-12 h-12 rounded-lg bg-slate-50 dark:bg-slate-800 border group-hover:bg-[#BD4234] group-hover:text-white transition-colors"><span className="text-[9px] font-bold uppercase opacity-70">Ano</span><span className="text-sm font-black">{exam.ano}</span></div>
                ...
                <div className="space-y-1 mt-4"><p className="text-[10px] font-semibold text-slate-400 uppercase flex items-center gap-1"><BookOpen size={12} /> Banca</p><h3 className="text-lg font-bold text-slate-800 dark:text-white line-clamp-2">{exam.origem}</h3></div>
              </div>
              <div className="relative z-10 w-full pt-4 mt-auto border-t border-slate-100 dark:border-slate-800">
                 ...
                 <div className="w-full h-1.5 bg-slate-100 rounded-full overflow-hidden"><div className={`h-full ${exam.isComplete ? 'bg-emerald-500' : 'bg-[#BD4234]'}`} style={{ width: `${(exam.userTotalDone / exam.count) * 100}%` }} /></div>
              </div>
            </button>
          ))}
        </div>
...
// ============================================================================
// 5. QUESTION CARD (100% INTACTO COM PREMIUM LOCK)
// ============================================================================
const CATEGORY_MAP: Record<string, string> = { 'CB': 'Ciclo Básico', 'CC': 'Clínica Cirúrgica', 'CM': 'Clínica Médica', 'GO': 'Ginecologia e Obstetrícia', 'PED': 'Pediatria', 'SC': 'Saúde Coletiva', 'PREV': 'Medicina Preventiva' };

const JustificationAccordion = ({ letter, text }: any) => {
    ...
    return (
        <div className="border border-zinc-200 dark:border-zinc-800 rounded-xl overflow-hidden bg-white dark:bg-zinc-900 mb-3 shadow-sm">
            <button onClick={() => setIsOpen(!isOpen)} className="w-full flex items-center justify-between p-4 hover:bg-zinc-100 transition-colors">
                <div className="flex items-center gap-3"><span className="flex items-center justify-center w-8 h-8 rounded-full bg-rose-100 text-rose-600 text-sm font-bold">{letter}</span><span className="text-sm font-semibold text-zinc-700 dark:text-zinc-300">Por que a {letter} está incorreta?</span></div>
...
const PremiumLockModal = ({ onNavigate }: any) => (
  <div className="fixed inset-0 z-[200] flex items-center justify-center p-4 bg-slate-900/90 backdrop-blur-md">
      <div className="bg-white dark:bg-slate-900 w-full max-w-md rounded-[2rem] shadow-2xl overflow-hidden text-center relative border border-slate-200 dark:border-slate-800">
          <div className="p-8 text-white bg-[#C04A3A]">
              <div className="w-16 h-16 bg-white/20 backdrop-blur-sm rounded-full flex items-center justify-center mx-auto mb-4"><Sparkles size={32} /></div>
          ...
          <div className="p-8 space-y-6">
              <p className="text-slate-600 dark:text-slate-300 text-sm leading-relaxed">Você atingiu o limite de <strong>10 questões diárias</strong> do plano gratuito. Para continuar praticando sem limites, torne-se Premium.</p>
              <div className="space-y-3">
                  ...
                  <button onClick={() => onNavigate && onNavigate('home')} className="block w-full py-4 text-slate-500 font-bold text-xs hover:text-slate-800">Voltar para o Início</button>
              </div>
          </div>
      </div>
  </div>
...
const QuestionCard: React.FC<any> = ({ question, currentIndex, totalQuestions, onNext, onAnswer, savedState, onStateChange, onBack, isLastQuestion, onFinish, onNavigate }) => {
  ...
  useEffect(() => {
    ...
    else { setSelectedOption(null); setIsSubmitted(false); setEliminatedOptions([]); setHighlights([]); setShowPremiumModal(false); }
    const allNotes = JSON.parse(localStorage.getItem('questbank_user_notes') || '{}');
    setUserNote(allNotes[question.id] || ''); setShowNotes(!!allNotes[question.id]);
  ...
  const isUserCorrect = useMemo(() => !selectedOption ? false : isAnulada || correctKeys.includes(selectedOption.toUpperCase()), [selectedOption, isAnulada, correctKeys]);
  ...
  const optionsList = useMemo(() => {
    ...
    return Object.entries(map).map(([key, text]) => ({ key, text }));
  }, [question]);
  ...
  const handleSubmit = () => {
    ...
    setIsSubmitted(true); setIsCommentExpanded(true);
  ...
  const handleTextMouseUp = useCallback(() => {
    ...
    const preRange = range.cloneRange(); preRange.selectNodeContents(enunciadoRef.current); preRange.setEnd(range.startContainer, range.startOffset);
    const start = preRange.toString().length; const text = range.toString(); const end = start + text.length;
  ...
  const renderedEnunciado = useMemo(() => {
    ...
    highlights.forEach((h, idx) => {
      ...
      parts.push(<mark key={`m-${idx}`} className="bg-yellow-200/60 rounded px-0.5 cursor-pointer hover:bg-red-200 transition-colors" onClick={(e) => { e.stopPropagation(); setHighlights(prev => prev.filter((_, i) => i !== idx)); }}>{text.substring(h.start, safeEnd)}</mark>);
      lastIndex = safeEnd;
    });
  ...
  const getOptionStyles = (key: string) => {
    ...
    if (isSubmitted) {
        if (isAnulada) { container += isSelected ? "bg-blue-50 border-blue-300" : "opacity-70 bg-white border-zinc-100"; circle += "border-zinc-300 text-zinc-400"; textStyle += "text-zinc-500"; } 
        else if (isCorrectKey) { container += "bg-emerald-50 border-emerald-500"; circle += "bg-emerald-600 border-emerald-600 text-white"; textStyle += "text-emerald-900 font-bold"; } 
         ...
         <div className="w-full max-w-[900px] p-6 md:py-12 space-y-8">
            <div className="flex flex-col gap-4 w-full">
                <div className="flex justify-between items-center">
                      <span className="font-mono text-xs font-bold text-zinc-400 bg-zinc-100 px-3 py-1.5 rounded-lg tracking-wider">{String(currentIndex + 1).padStart(2, '0')} / {totalQuestions}</span>
                      <div className="flex items-center gap-1 bg-white p-1 rounded-2xl border border-zinc-100 shadow-sm">
                         {highlights.length > 0 && <button onClick={() => setHighlights([])} className="p-2 text-zinc-400 hover:text-rose-500 rounded-xl"><Eraser size={18}/></button>}
                         <button onClick={() => setShowNotes(!showNotes)} className={`p-2 rounded-xl ${showNotes || userNote ? 'text-amber-600 bg-amber-50' : 'text-zinc-400 hover:bg-zinc-50'}`}><StickyNote size={18}/></button>
                      </div>
                </div>
            ...
            <div ref={enunciadoRef} onMouseUp={handleTextMouseUp} className="text-lg md:text-[1.25rem] leading-[1.8] text-zinc-800 font-medium cursor-text tracking-tight">{renderedEnunciado}</div>
            
            <img src={`https://res.cloudinary.com/dfly2thac/image/upload/apex-med/${question.id}.jpg`} alt="" className="mt-6 max-w-full rounded-xl border border-zinc-200 shadow-sm" data-attempt="0" onError={(e) => { const t = e.currentTarget; const f = ['.jpg', '.png']; let a = parseInt(t.getAttribute('data-attempt')||'0'); if (a < f.length-1) { t.src = t.src.replace(f[a], f[a+1]); t.setAttribute('data-attempt', String(a+1)); } else t.style.display = 'none'; }}/>
            ...
            {showNotes && (
               <div className="bg-amber-50 border border-amber-100 p-5 rounded-2xl">
                  ...
                  <textarea value={userNote} onChange={e => setUserNote(e.target.value)} onBlur={() => { const n = JSON.parse(localStorage.getItem('questbank_user_notes') || '{}'); if(userNote.trim()) n[question.id]=userNote; else delete n[question.id]; localStorage.setItem('questbank_user_notes', JSON.stringify(n)); }} className="w-full bg-transparent border-none focus:ring-0 text-zinc-800 resize-none h-24 text-sm" placeholder="Suas anotações..."/>
               </div>
            )}
               ...
               {optionsList.map((opt) => {
                   ...
                   return (
                     <div key={opt.key} onClick={() => !isEliminated && !isSubmitted && setSelectedOption(opt.key)} className={styles.container}>
                        <div className={styles.circle}>{isEliminated ? <EyeOff size={14}/> : isSubmitted && correctKeys.includes(opt.key) ? <CheckCircle2 size={16}/> : isSubmitted && selectedOption===opt.key ? <XCircle size={16}/> : opt.key}</div>
                        <div className={styles.text}>{opt.text}</div>
                        {!isSubmitted && (
                            <button onClick={(e) => { e.stopPropagation(); setEliminatedOptions(p => p.includes(opt.key) ? p.filter(x=>x!==opt.key) : [...p, opt.key]); }} className={`absolute right-4 p-2 rounded-full border transition-all ${isEliminated ? "opacity-100 bg-white text-zinc-400" : "bg-white border-zinc-100 text-zinc-300 opacity-0 md:group-hover:opacity-100"}`}>
            ...
            {isSubmitted && (
               <div className="mt-8 rounded-[2rem] bg-zinc-50/50 border border-zinc-200 overflow-hidden">
                  ...
                  <div className="p-6 md:p-8 space-y-6">
                      ...
                      {!isAnulada && optionsList.map(opt => !correctKeys.includes(opt.key) && (question as any)[`justificativa_${opt.key.toLowerCase()}`] ? <JustificationAccordion key={opt.key} letter={opt.key} text={(question as any)[`justificativa_${opt.key.toLowerCase()}`]} /> : null)}
                  </div>
               </div>
            )}
            ...
            <div className="fixed bottom-6 left-1/2 -translate-x-1/2 flex items-center gap-3 bg-white/90 backdrop-blur-xl p-2 rounded-full shadow-2xl border border-zinc-100 z-40">
               ...
               {!isSubmitted ? (
                  <button onClick={handleSubmit} disabled={!selectedOption || isSubmitting} className="bg-zinc-900 text-white px-10 py-3 rounded-full font-bold text-sm disabled:opacity-50 hover:scale-[1.02] shadow-xl flex gap-2">
                      {isSubmitting ? 'Salvando...' : 'Responder'}
                  </button>
               ) : (
                  onNext && !isLastQuestion ? <button onClick={onNext} className="bg-indigo-600 text-white px-10 py-3 rounded-full font-bold text-sm flex items-center gap-2">Próxima <ChevronRight size={18}/></button>
                  : <button onClick={onFinish} className="bg-rose-600 text-white px-10 py-3 rounded-full font-bold text-sm flex items-center gap-2">Finalizar <AlertCircle size={18}/></button>
               )}
            </div>
         </div>
      </div>
...
// 6. QUIZ RESULT MOCK
// ============================================================================
const QuizResultScreen = ({ questions, sessionState, onRetry, onExit }: any) => {
  const correct = questions.filter((q:any) => sessionState[q.id]?.selectedOption === q.correta).length;
...
export default function App() {
  ...
  const handleStartQuiz = (limit: number, randomize: boolean, smart: boolean, ids?: number[]) => {
    ...
    setSessionState({}); setCurrentPage(1); timerRef.current = 0; setQuizStatus('active');
  ...
  return (
    <>
      {quizStatus === 'finished' ? (
        <QuizResultScreen questions={activeQuestions} sessionState={sessionState} onExit={() => { setQuizStatus('idle'); setCurrentView('home'); }} />
      ) : quizStatus === 'active' ? (
        <div className="h-screen w-screen bg-slate-50 transition-colors duration-300 font-sans flex flex-col overflow-hidden">
          {!settings.focusMode && (
            <header className="flex-none bg-white border-b border-slate-200 px-4 h-14 flex items-center justify-between z-50">
              <div className="flex items-center gap-2"><span className="font-bold text-slate-700 hidden sm:inline">Simulado</span><QuizTimer isRunning={true} onTimeUpdate={(t:number) => timerRef.current = t} /></div>
              <button onClick={() => setQuizStatus('idle')} className="p-2 hover:bg-[#BD4234]/10 text-[#BD4234] rounded-lg"><X size={20} /></button>
            </header>
          )}
          <main className="flex-1 w-full relative overflow-hidden">
            {activeQuestions[currentPage - 1] && (
              <QuestionCard 
                question={activeQuestions[currentPage - 1]} currentIndex={currentPage - 1} totalQuestions={activeQuestions.length} 
                savedState={sessionState[activeQuestions[currentPage - 1].id]}
                onStateChange={(s:any) => setSessionState(p => ({ ...p, [activeQuestions[currentPage - 1].id]: s }))}
                onAnswer={() => { setHistory(localDb.getHistory()); }} 
                onNext={() => setCurrentPage(p => p + 1)} onBack={currentPage > 1 ? () => setCurrentPage(p => p - 1) : undefined}
                isLastQuestion={currentPage === activeQuestions.length} onFinish={() => setQuizStatus('finished')} onNavigate={setCurrentView}
              />
            )}
          </main>
        </div>
        ...
        <div className="min-h-screen bg-slate-50 pb-20 font-sans overflow-x-hidden">
          <main className={`animate-in duration-500 ease-in-out ${slideDirection === 'right' ? 'slide-in-from-right-10 fade-in' : 'slide-in-from-left-10 fade-in'}`}>
             {currentView === 'home' && <FilterScreen questions={metadata} history={history} filters={filters} setFilters={setFilters} onStartQuiz={handleStartQuiz} />}
             {currentView === 'exams' && <ExamsScreen questions={metadata} history={history} onStartExam={handleStartExam} />}
          </main>
          ...
          <div className="fixed bottom-0 left-0 right-0 bg-white/80 backdrop-blur-xl border-t border-slate-200 z-30 transition-all">
            <div className="flex justify-around items-center h-16 max-w-md mx-auto">
              <button onClick={() => changeView('home')} className={`p-2 rounded-xl transition-all ${currentView === 'home' ? 'text-[#BD4234] bg-red-50 scale-110' : 'text-slate-400'}`}><Home size={24} /></button>
              <button onClick={() => changeView('exams')} className={`p-2 rounded-xl transition-all ${currentView === 'exams' ? 'text-[#BD4234] bg-red-50 scale-110' : 'text-slate-400'}`}><FileText size={24} /></button>
            </div>
          </div>
        </div>
      )}
